        ...
        <div className="grid grid-cols-1 ps-4">
          {messagesData.map((item) => (
            <div
              ...
              className="group grid cursor-pointer grid-cols-[auto_minmax(0,1fr)] gap-2.5 rounded-md px-2 py-2.5 pe-3 transition-colors hover:bg-gray-100"
            >
              <div className={cn('relative', item.avatar.length > 1 && 'me-1')}>
                ...
                {item.avatar.length > 1 && (
                  <Avatar
                    src={item.avatar[1]}
                    name={item.name}
                    className="absolute -bottom-1 end-1.5 !h-9 !w-9 border-2 border-gray-0"
                  />
                )}
              </div>
...
export default function MessagesDropdown({
  ...
  return (
    ...
    >
      ...
      <Popover.Content className="z-[9999] pb-6 pe-6 ps-0 pt-5 [&>svg]:hidden sm:[&>svg]:inline-flex">
